...
@app.route('/fill_form/<template>')
def fill_form(template):
    return render_template(f'resume_templates/{template}.html', template=template)

@app.route('/generate_resume', methods=['POST'])
def generate_resume():
    template = request.form.get("template")
    data = request.form.to_dict()
    return render_template('resume_preview.html', data=data, template=template)

@app.route('/download_resume', methods=['POST'])
def download_resume():
    template = request.form.get("template")
    data = request.form.to_dict()
    html = render_template('resume_preview.html', data=data, template=template)
